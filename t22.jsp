...
try {
    ...
    // Convert clickTime to GMT+8
    $dateTime = new DateTime($clickTime, new DateTimeZone('UTC'));
    $dateTime->setTimezone(new DateTimeZone('Asia/Taipei'));
    $clickTimeGMT8 = $dateTime->format('Y-m-d H:i:s');

    $sql = "INSERT INTO UserPageAccess (IP, Username, PageName, AccessTime) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->execute([$ip, $user, $page, $clickTimeGMT8]);
